            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <section className="space-y-8">
                    ...
                    <div className="bg-cde-bg-light border border-cde-border p-8 rounded-xl animate-slide-up shadow-sm hover:shadow-md transition-shadow" style={{ animationDelay: '300ms' }}>
                        ...
                        <div className="flex flex-wrap gap-2 mb-6">
                            {metricsInfo.map(m => (
                                <button 
                                    ...
                                    className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest transition-all ${selectedMetric === m.name ? 'bg-cde-text text-cde-bg shadow-[0_0_10px_rgba(252,211,209,0.4)]' : 'bg-cde-bg-lighter text-cde-text-muted border border-cde-border hover:border-cde-text hover:text-cde-text'}`}
                                >
                                    {m.name}
                                </button>
                            ))}
                        ...
                        <div className="border border-cde-text/30 bg-cde-text/5 p-6 rounded-lg border-l-4 border-l-cde-text animate-fade-in min-h-[140px]">
                            <h4 className="font-bold text-cde-text mb-3 uppercase tracking-wide text-sm flex justify-between">
                                <span>{activeMetric.name}</span>
                                <span>{activeMetric.score}/100</span>
                            </h4>
                            <p className="text-sm text-cde-text-muted leading-relaxed mb-3">
                                <strong>¿Cómo se mide?</strong> {activeMetric.calc}
                            </p>
                            <p className="text-sm text-white/90 leading-relaxed bg-cde-bg-lighter/50 p-3 rounded border border-cde-border/50">
                                <strong>Táctica de Mejora:</strong> {activeMetric.improvement}
                            </p>
                        </div>
                    </div>
                </section>
